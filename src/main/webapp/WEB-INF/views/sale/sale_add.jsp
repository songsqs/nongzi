 ...
 -->
	...
	<div class="box span12">
		<div class="box-header" data-original-title>
			<h2>
				<i class="halfings-icon edit"></i> <span class="break"></span>
				添加销售记录
			</h2>
		</div>
	...
	<script type="text/javascript">
			...
			$.ajax({
						...
						success : function(data) {
							...
							$("<option></option>").attr('disabled', 'disabled')
									.attr('selected', 'selected').text("请选择产品")
									.appendTo(obj);
		...
		function onCustomerInput(customerName) {
			$.ajax({
						...
						success : function(data) {
							...
							$("<option></option>").attr('disabled', 'disabled')
									.attr('selected', 'selected').text("请选择客户")
									.appendTo(obj);
		...
		function onSubmit() {
			...
			if (!checkAndGetSelectValue("numId", "数量不能为空", "0")
					|| !checkIsNaN("numId", "数量必须是数字")) {
				return false;
			}
			if (!checkAndGetSelectValue("totalPriceId", "总价不能为空", "0")
					|| !checkIsNaN("totalPriceId", "总价必须为数字")) {
				return false;
			}
			if (!checkAndGetSelectValue("profitId", "利润不能为空", "0")
					|| !checkIsNaN("profitId", "利润必须是数字")) {
				return false;
			}
